...
namespace FLORENCE.Frame.Cli.Dat.In
{
    public class Player
    {
        ...
        public void Move_Backwards(FrameEventArgs e)
        {
            ...
            Framework.GetClient().GetData().GetInputBuffer(!Framework.GetClient().GetData().GetInBufferToWrite()).GetPlayer().Set_PlayerPosition(
                temp
            );
        }
        ...
        {
            if (Framework.GetClient().GetData().GetInBufferToWrite() == false)
            {
                return (Int16)0;
            }
            else
        ...
        {
            Framework.GetClient().GetData().GetInputBuffer(!Framework.GetClient().GetData().GetInBufferToWrite()).GetPlayer().Set_MousePos(pos);
            
            // Calculate the offset of the mouse position
            var deltaX = Framework.GetClient().GetData().GetInputBuffer(!Framework.GetClient().GetData().GetInBufferToWrite()).GetPlayer().GetMousePos().X - mousePos.X;
            var deltaY = Framework.GetClient().GetData().GetInputBuffer(!Framework.GetClient().GetData().GetInBufferToWrite()).GetPlayer().GetMousePos().Y - mousePos.Y;
